...
ARGs_df = df.loc[:,"aap/aspU":"ykgK/ecpR"]
...
major_ARGs_df = major_ARGs_df.loc[:,"variants":"ykgK/ecpR"]
major_ARGs_df = major_ARGs_df.loc[:,major_ARGs_df.sum()!=0]
major_ARGs_df.set_index("variants",inplace=True)
major_ARGs_df.to_csv(r"major_VFs.csv")
print("三个变异子共有毒力基因数目",len(major_ARGs_df.columns))
...
venn_figure = venn(venn_dict3,
               ...
               alpha=0.8)
plt.tight_layout()
plt.savefig(r"fig4_OXA_VGs_venn.svg",dpi=600,fmt="svg")
plt.show()
...
plt.tight_layout()
plt.savefig(r"OXA毒力基因数目小提琴图_专业版.svg", dpi=600, format="svg")
plt.show()
